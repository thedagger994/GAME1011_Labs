...
string Knight::specialName = "Shield Bash";
...
string Wizard::specialName = "Meteor";

struct Orc : public Enemy
{
	static string species;
	array<string, 2> taunts { "You small!", "Me strong!" };
...
{
	static string species;
	array<string, 2> taunts { "For the Lich King!", "Join Us!" };
	virtual void TauntPlayer () override { cout << taunts[rand () % 2] << endl; }
};
...
int main ()
{
	...
	cout << "Press O to fight Orc, U to fight Undead." << endl;
	cin >> playerType;
	if (toupper (playerType) == 'O') enemy = new Orc;
	...
	while (player->health > 0 && enemy->health > 0)
	{
		...
		enemy->health -= player->SpecialAttack ();
		enemy->TauntPlayer ();
		cout << "Health after this round:\n Player: " << player->health << "\n Enemy: " << enemy->health << endl;
	}
	if (player->health > 0) cout << player->name << " Wins!" << endl;
	else cout << player->name << " Died!" << endl;
